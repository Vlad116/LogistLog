...
//            ", company_role = ?" +
    ...
    //language=SQL
    private static final String SQL_SET_REQUISITE_ID = "UPDATE company SET requisite_id = ? WHERE id = ?";
...
//    @Lazy
    ...
    private RowMapper<Company> companyRowMapper = (resultSet, i) -> Company.builder()
            ...
            .companyRole(resultSet.getString("company_role"))
    ...
    public void save(Company model) {
        ...
        jdbcTemplate.update(
                connection -> {
                    ...
                    ps.setString(4, model.getCompanyRole());
...
//                    ps.setString(4, model.getCompany_role());
    ...
    @Override
    public void setResuisiteID(Long requisiteId, Long companyId) {
        jdbcTemplate.update(
                connection -> {
                    PreparedStatement ps =
                            connection.prepareStatement(SQL_SET_USER_ID);
                    ps.setLong(1, requisiteId);
                    ps.setLong(2, companyId);
                    return ps;
                });
    }
    ...
    public Optional<Company> findOneByID(Long id) {
        return Optional.ofNullable(jdbcTemplate.queryForObject(SQL_SELECT_COMPANY_BY_ID, companyRowMapper, id));
    }
