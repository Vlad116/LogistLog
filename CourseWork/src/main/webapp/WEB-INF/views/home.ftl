...
<div class="col-lg-12" id="applicationsDiv" style="padding-top: 5%">
    ...
    <p>You can find yourself an application, and respond to it by clicking on the "respond" button,
        or by sending an email to the company email using the "send email" button.
    </p>
                   ...
                   <#--добавляет в мои зявки -->
                        <td>
                            <button type="button" class="btn btn-outline-success"
                                    onclick="respondApplication(${application.id})">Respond
                            </button>
                        </td>
                     <#--<#else>-->
...
<div class="col-lg-12" id="carriersTable" style="padding-top: 5%">
    ...
    <p>If you are a customer, you can see the list of available carriers.</a></p>
...
<script>
        ...
        $.ajax({
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                contentTableHTML += "<td>\n" + "<button type=\"button\" class=\"btn btn-outline-info\" data-toggle=\"modal\"\n" +
                    ...
                    "\n" +
                    "            <td>\n" +
                    "                <button type=\"button\" class=\"btn btn-outline-success\" onclick=\"respondApplication(" + data[i].id + ")\">Respond</button>\n" +
                    "            </td>\n";
                contentTableHTML += "</tr>";
            }
    ...
    function respondApplication(id) {
        $.ajax({
            type: 'post',
            url: '/respondapplication',
            data: {
                id: id
            }
        }).done(function (data) {
